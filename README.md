...
> Dark-themed login with email/password authentication and Microsoft Entra ID support.
>
> 다크 테마 로그인 — 이메일/비밀번호 인증 및 Microsoft Entra ID 지원.
...
> Bilingual admin gate (Korean + English) with secure admin key access.
>
> 이중 언어 관리자 게이트 (한국어 + 영어) — 관리자 키로 안전하게 접근.
...
| � [docs/OPERATIONS.md](docs/OPERATIONS.md) | **Operations & deployment** — Start/stop, where it runs, showcase vs production repo, CI/CD setup / 운영 & 배포 — 시작/중지, 실행 위치, 전시 vs 프로덕션 repo, CI/CD 설정 || 🔐 [docs/SECURITY.md](docs/SECURITY.md) | **Secrets & security** — Where passwords are stored, two-layer protection, OIDC auth, key rotation / 시크릿 & 보안 — 비밀번호 저장 위치, 2중 보호, OIDC 인증, 키 교체 / 密钥与安全 — 密码存储位置、双层保护、OIDC认证、密钥轮换 |
| 📝 [docs/DEVELOPMENT_JOURNAL.md](docs/DEVELOPMENT_JOURNAL.md) | **Dev journal** — Full build process, pitfalls, prompt techniques, lessons learned / 개발 일지 — 전체 구축 과정, 함정, 프롬프트 기법, 교훈 / 开发日志 — 完整构建流程、踩坑记录、Prompt技巧、经验总结 || �📋 [bmad-docs/PRD.md](bmad-docs/PRD.md) | Product Requirements Document / 제품 요구사항 문서 |

...
| 🔐 [docs/SECURITY.md](docs/SECURITY.md) | **Secrets & security** — Where passwords are stored, two-layer protection, OIDC auth, key rotation / 시크릿 & 보안 — 비밀번호 저장 위치, 2중 보호, OIDC 인증, 키 교체 / 密钥与安全 — 密码存储位置、双层保护、OIDC认证、密钥轮换 |
| 📝 [docs/DEVELOPMENT_JOURNAL.md](docs/DEVELOPMENT_JOURNAL.md) | **Dev journal** — Full build process, pitfalls, prompt techniques, lessons learned / 개발 일지 — 전체 구축 과정, 함정, 프롬프트 기법, 교훈 / 开发日志 — 完整构建流程、踩坑记录、Prompt技巧、经验总结 |

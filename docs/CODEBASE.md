...
| `startup.sh` | Azure App Service startup script — installs system deps (`zstd`, `ffmpeg`), activates `antenv` virtual environment, launches `uvicorn --workers 2` / Azure 시작 스크립트 (시스템 의존성 설치 포함) |
...
- `get_diagnostics()` returns masked URL, scheme, auth mode, and last error for health checks / 진단 정보 반환
...
|---|---|---|---|
| `/api/auth/send-code` | POST | ❌ | Send 6-digit verification code to email (rate limit: 1/60s, TTL 5min) / 이메일로 6자리 인증코드 발송 |
| `/api/auth/register` | POST | ❌ | Verify code + create user (bcrypt hash), return JWT / 코드 확인 + 사용자 생성, JWT 반환 |
| `/api/auth/login` | POST | ❌ | Verify credentials, return JWT / 인증 정보 확인, JWT 반환 |
| `/api/auth/microsoft` | POST | ❌ | Microsoft Entra ID login (validate ID token, auto-register) / Microsoft 로그인 |
...
|---|---|---|
| Auth / 인증 | `SendCodeRequest`, `UserRegister`, `UserLogin`, `MicrosoftAuthRequest`, `TokenResponse`, `UserProfile` | Email verification, registration, login, Microsoft SSO, profile / 이메일 인증, 회원가입, 로그인, Microsoft SSO, 프로필 |
...
| **SpeechService** | `speech_service.py` | Azure Speech REST API for STT, TTS, pronunciation. Uses ffmpeg (or Python fallback) for audio format conversion / 음성인식, 합성, 발음평가 |
...
| **EmailService** | `email_service.py` | Email verification codes via Azure Communication Services (ACS) REST API. Stores codes in Redis (5min TTL), rate-limits 1/60s per email / 이메일 인증코드 (ACS REST API) |
...
| 7 | `get_drama_dialogue` | Real K-drama workplace dialogues — **async**, queries Cosmos DB first with JSON file fallback / 한국 드라마 직장 대화 (Cosmos DB 우선, JSON 폴백) |
...
**Data source / 데이터 소스**: Tools 1-6, 8-9 read from `data/business_korean.json`. Tool 7 queries **Cosmos DB** `drama_content` container first, falling back to JSON.

데이터 소스: 도구 1-6, 8-9는 JSON 파일, 도구 7은 Cosmos DB 우선 조회 후 JSON 폴백.
...
|---|---|---|
| `index.html` | Login / Register / 로그인 / 회원가입 | Email + password auth with verification code, Microsoft Entra ID login, redirect to chat on success / 이메일+비밀번호+인증코드 인증, Microsoft 로그인 |
...
| `seed_drama_cosmos.py` | Seeds `drama_dialogues` from `business_korean.json` into Cosmos DB `drama_content` container. Uses deterministic uuid5 IDs for upsert safety. Run: `python -m scripts.seed_drama_cosmos` / JSON 드라마 데이터를 Cosmos DB에 시딩 |
